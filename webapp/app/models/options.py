...
class Option:
    ...
    `cost` oyuncuya gösterilen METİNdir ("2 fişek + gürültü"); `spend` ise aynı
    bedelin MAKİNE-OKUNUR hali ({"9mm fişek": 2}). Sunucu envanteri `spend`'e
    göre keser — anlatıcının yanıtından "silah sıkıldı mı" diye okumaya gerek
    kalmaz, çünkü ne harcanacağı sahne yazılmadan ÖNCE bellidir.
    """
